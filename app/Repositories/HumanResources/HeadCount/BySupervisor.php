 ...
 * summary.
 */
class BySupervisor extends HumanResources implements HumanResourcesInterface
{
    protected $type;
    ...
    {
        return Supervisor::orderBy('name')->withCount(
    ...
    {
        return Supervisor::orderBy('name')->with(
            ['employees' => function ($query) use ($status, $site, $by_site) {
                return $by_site ?
                    $query->$status()
                    ->with('termination')
