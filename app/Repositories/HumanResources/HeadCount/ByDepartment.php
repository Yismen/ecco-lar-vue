 ...
 * summary.
 */
class ByDepartment extends HumanResources implements HumanResourcesInterface
{
    protected $type;
    ...
    {
        return Department::orderBy('name')->withCount(
    ...
    {
        return Department::orderBy('name')->with(
            ['employees' => function ($query) use ($status, $site, $by_site) {
                return $by_site ?
                    $query->$status()
                    ->with('termination')
