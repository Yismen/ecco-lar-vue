 ...
 * summary.
 */
class ByNationality extends HumanResources implements HumanResourcesInterface
{
    protected $type;
    ...
    {
        return Nationality::orderBy('name')->withCount(
    ...
    {
        return Nationality::orderBy('name')->with(
            ['employees' => function ($query) use ($status, $site, $by_site) {
                return $by_site ?
                    $query->$status()
                    ->with('termination')
