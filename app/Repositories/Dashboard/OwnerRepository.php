    ...
    public function toArray()
    {
        ...
        return [
            ...
            'performance' => [
                ...
                'revenue' => [
                    $data_repo->toArray([
                        'data' => number_format($data->map->revenue, 2),
                        'label' => 'Revenue'
                    ])
                ],
                'rph' => [
                    $data_repo->toArray([
                        'data' => number_format($data->map->rph, 2),
                        'label' => 'RPH',
                        'fill' => false
                    ])
                ],
                'login_time' => [
                    $data_repo->toArray([
                        'data' => number_format($data->map->login_time, 2),
                        'label' => 'Login Hours'
                    ])
                ],
                'production_time' => [
                    $data_repo->toArray([
                        'data' => number_format($data->map->production_time, 2),
                        'label' => 'Prod. Hours'
                    ])
                ],
                'sph' => [
                    $data_repo->toArray([
                        'data' => number_format($data->map->sph_goal, 2),
                        'label' => 'Goal',
                        'borderColor' => 'rgba(211,47,47 ,1)',
                        'backgroundColor' => 'rgba(211,47,47 ,.25)',
                    ]),
                    $data_repo->toArray([
                        'data' => number_format($data->map->sph, 2),
                        'fill' => false,
                        'label' => 'SPH'
                    ])
                ],
                'efficiency' => [
                    $data_repo->toArray([
                        'data' => number_format($data->map->efficiency, 2),
                        'label' => 'Efficiency',
                        'fill' => false
                    ])
                ]
